...
export default function DevPage() {
  ...
  const setRole = (role: string) => {
    ...
    else if (role === "staff1") router.push("/staff/settings");
  };
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8 max-w-sm w-full">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full bg-orange-600 hover:bg-orange-700 text-white font-medium py-2.5 rounded-lg text-sm transition-colors"
          >
            เข้าในฐานะ กิจการนิสิต
          </button>
          <button
            onClick={() => setRole("staff")}
            className="w-full bg-red-600 hover:bg-orange-700 text-white font-medium py-2.5 rounded-lg text-sm transition-colors"
          >
